...
# Get backend URL from environment or detect deployment
def get_backend_url():
    """Get the correct backend URL based on environment"""
    
    # First check for explicit BACKEND_URL
    if os.getenv('BACKEND_URL'):
        return os.getenv('BACKEND_URL')
    
    # Check if we're running on Render
    elif os.getenv('RENDER'):
        # On Render, try to detect the backend port
    ...
    # Check if we're running on Heroku
        ...
        # On Heroku, backend and frontend are on same service
        return 'http://localhost:8000'
    
    # Check if we're running on Railway
    elif os.getenv('RAILWAY_ENVIRONMENT'):
        # On Railway, use localhost for same service
        return 'http://localhost:8000'
